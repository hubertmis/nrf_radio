                ...
                if let Ok(frame) = frame {
                    Rx::use_data_from_context(context, |d| {
                        ...
                        if filter_passed && ack_requested {
                            ...
                            // TODO: Get TX ramp up time (40) from Phy
                            let target_tx_time = Timestamp::wrapping_add(phyend_timestamp, 192);
                            let target_tx_start_time = Timestamp::wrapping_sub(target_tx_time, 40);
                            let timer_result =
                            ...
                            let tx_result = d.phy.tx(TxRequest::new(
                                &ack_frame,
                                Rx::phy_tx_ack_callback,
                                context,
                            )
                            .at_event(ppi_ch));
                            ...
                            if d.timer
                                ...
                                .unwrap()
                            {
                                // TODO: check with Phy if request was triggered. If not, notify RX
                                // success immediately
                            }
                        } else {
